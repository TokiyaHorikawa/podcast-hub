...
async function main() {
  try {
    ...
    const user1 = await _prisma.user.create({
      data: {
    ...
    const user2 = await _prisma.user.create({
      data: {
    ...
    // user1のPodcast作成
    const podcast1 = await _prisma.podcast.create({
      data: {
        title: 'スタートアップラジオ',
    ...
    // user2のPodcast作成
    const podcast2 = await _prisma.podcast.create({
      data: {
        title: 'テック探求ラボ',
