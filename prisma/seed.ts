...
async function main() {
  try {
    ...
    // podcast1のエピソード作成
    const { error: episodes1Error } = await supabase
      .from('episodes')
      .insert([
        {
    ...
    // podcast2のエピソード作成
    const { error: episodes2Error } = await supabase
      .from('episodes')
      .insert([
        {
    ...
    // コンテンツ作成
    const { error: content1Error } = await supabase
      .from('contents')
      .insert({
    ...
    const { error: content2Error } = await supabase
      .from('contents')
      .insert({
